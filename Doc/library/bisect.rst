...
.. function:: bisect_left(a, x, lo=0, hi=len(a), key=None)
   ...
   The parameter *key* specifies how to extract value from item in list. By default
   *key* is None.
...
of the record in question::
    ...
    >>> data[bisect_left(data, 0, key=lambda r: r[1])]
    ('black', 0)
    >>> data[bisect_left(data, 1, key=lambda r: r[1])]
    ('blue', 1)
    >>> data[bisect_left(data, 5, key=lambda r: r[1])]
    ('red', 5)
    >>> data[bisect_left(data, 8, key=lambda r: r[1])]
    ('yellow', 8)
